...
class AuthController extends Controller
{
    public function login(Request $request)
    {
        try {
            ...
            // Log error to ApiLog instead of Log
            $response = ['message' => 'An error occurred during login', 'error' => $e->getMessage()];
            $this->logAPICalls('login', $request->email, $request->all(), $response); // Log API call
    ...
    public function insertSession(Request $request)
    {
        try {
            $request->validate([
                'user_id' => 'get|string|exists:user_accounts,id' // Ensure this table name is correct
            ]);
